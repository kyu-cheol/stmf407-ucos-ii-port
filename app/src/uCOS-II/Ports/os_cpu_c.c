...
#if OS_TMR_EN > 0u
static  INT16U  OSTmrCtr __attribute__((unused));
#endif
...
void  OS_CPU_SysTickInit (INT32U  cnts)
{
    ...
    *((volatile INT32U *)0xE000E018uL) = 0;
                                                                /* Enable timer.                                        */
...
//     // 1. 카운터 리로드 값 설정
//     OS_CPU_CM_SYST_RVR    = cnts - 1u;

//     // 2. 🚨 SysTick 우선순위를 무조건 최하위 우선순위(Lowest, 15)로 설정
//     // Cortex-M4(STM32F4)의 상위 4비트 기준 최하위는 0xF0 입니다.
//     prio                  = OS_CPU_CM_SCB_SHPRI3;
//     prio                 &= 0x00FFFFFFu; // SysTick 영역(최상위 8비트) 초기화
//     prio                 |= (0x00u << 24u); // 0 우선순위 주입
//     OS_CPU_CM_SCB_SHPRI3  = prio;

//     // 3. 현재 카운트 값 초기화
//     *((volatile INT32U *)0xE000E018uL) = 0;

//     // 4. 🚨 [순서 교정] 클록 소스, 인터럽트 허용, 타이머 활성화를 '동시에' 한 번에 켭니다.
//     OS_CPU_CM_SYST_CSR   |= OS_CPU_CM_SYST_CSR_CLKSOURCE |
//                             OS_CPU_CM_SYST_CSR_TICKINT   | // 인터럽트를 타이머 활성화와 동시에 인입
//                             OS_CPU_CM_SYST_CSR_ENABLE;
// }
